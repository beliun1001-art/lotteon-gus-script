/**
 * LOTTEON Google Sheets Apps Script GitHub Remote Loader v1.13-PR21-SMOKE
 *
 * v1.10 핵심:
...
const LOTTEON_PR21_RAW_BASE = 'https://raw.githubusercontent.com/beliun1001-art/lotteon-gus-script/codex/issue-20-vat-business-half-summary/';

const LOTTEON_GITHUB_CODE_URL = LOTTEON_PR21_RAW_BASE + 'Code.gs';
const LOTTEON_GITHUB_PATCH_URL = LOTTEON_PR21_RAW_BASE + 'Patch_v6_24_bootstrap_auto_continue.gs';
...
const LOTTEON_LOADER_VERSION = 'v1.13-PR21-SMOKE';
...
function testLotteonGitHubConnection() {
  ...
  SpreadsheetApp.getUi().alert(
    ...
    'PR #21 Base: ' + LOTTEON_PR21_RAW_BASE + '\n' +
...
function loadLotteonRemoteBundle_() {
  ...
  const patchText = originalPatchText.replace(
    ...
    "var LOTTEON_PATCH_BASE_URL = '" + LOTTEON_PR21_RAW_BASE + "';"
  );
  if (patchText === originalPatchText) throw new Error('PR #21 bootstrap 내부 LOTTEON_PATCH_BASE_URL 교체에 실패했습니다.');
...
// v6.59+ 시간 기반 브랜드운영 continuation trigger용 로컬 bridge.
function continueBrandLifecycleDashboard_v659_() {
  return runRemoteFunctionByName_('continueBrandLifecycleDashboard_v659_');
}
