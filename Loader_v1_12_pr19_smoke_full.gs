/**
 * LOTTEON Google Sheets Apps Script GitHub Remote Loader v1.12-PR19-SMOKE
 *
 * v1.10 핵심:
...
const LOTTEON_PR19_HEAD = '3f030c1a573879f23c81a7bcd76ba18a0329eeef';
const LOTTEON_PR19_RAW_BASE = 'https://raw.githubusercontent.com/beliun1001-art/lotteon-gus-script/3f030c1a573879f23c81a7bcd76ba18a0329eeef/';

const LOTTEON_GITHUB_CODE_URL = LOTTEON_PR19_RAW_BASE + 'Code.gs';
const LOTTEON_GITHUB_PATCH_URL = LOTTEON_PR19_RAW_BASE + 'Patch_v6_24_bootstrap_auto_continue.gs';

// 필터별_상품수 v6.56 운영 경로는 PR #19 범위가 아니므로 main을 그대로 사용합니다.
...
const LOTTEON_LOADER_VERSION = 'v1.12-PR19-SMOKE';

function onOpen() {
  const ui = SpreadsheetApp.getUi();
  ui.createMenu('LOTTEON 자동화')
    .addItem('GitHub 코드 연결 테스트', 'testLotteonGitHubConnection')
    .addItem('PR #19 브랜드운영 이어실행 재개', 'continueBrandLifecycleDashboard_v659_')
    .addSeparator()
...
function testLotteonGitHubConnection() {
  ...
  SpreadsheetApp.getUi().alert(
    ...
    'PR #19 Head: ' + LOTTEON_PR19_HEAD + '\n' +
    'Patch 내부 Base: ' + LOTTEON_PR19_RAW_BASE + '\n' +
...
function loadLotteonRemoteBundle_() {
  ...
  const originalPatchText = fetchTextOrThrow_(LOTTEON_GITHUB_PATCH_URL, 'Patch bootstrap');

  // PR #19 bootstrap 파일 안의 main 주소를 정확한 Head SHA 주소로 임시 교체합니다.
  // 따라서 v6.52, v6.53, v6.58, v6.59 patch가 모두 같은 검수 Head에서 로드됩니다.
  const patchText = originalPatchText.replace(
    /var\s+LOTTEON_PATCH_BASE_URL\s*=\s*['"][^'"]+['"]\s*;/,
    "var LOTTEON_PATCH_BASE_URL = '" + LOTTEON_PR19_RAW_BASE + "';"
  );

  if (patchText === originalPatchText) {
    throw new Error('PR #19 bootstrap 내부 LOTTEON_PATCH_BASE_URL 교체에 실패했습니다.');
  }

  const bundle = codeText + '\n\n;\n\n' + patchText;
  PropertiesService.getScriptProperties().setProperty(
    'LOTTEON_REMOTE_LAST_VERSION',
    detectRemoteVersion_(bundle)
  );
  return bundle;
}
...
function refreshDashboardFastOnly() {
  ...
  const version = detectRemoteVersion_(bundle);
  if (version !== 'v6.59') {
    throw new Error('PR #19 운영 smoke 중단: v6.59가 아니라 ' + version + '이 로드되었습니다.');
  }
  return eval(
    bundle +
    '\n\n; if (typeof refreshDashboardFastOnly !== "function") {' +
    ' throw new Error("Remote function not found: refreshDashboardFastOnly");' +
    ' }\nrefreshDashboardFastOnly();'
  );
}

/**
 * PR #19 시간 기반 trigger가 호출할 로컬 bridge입니다.
 * 원격 v6.59 continuation handler를 같은 이름으로 실행합니다.
 */
function continueBrandLifecycleDashboard_v659_() {
  ...
  return eval(
    bundle +
    '\n\n; if (typeof continueBrandLifecycleDashboard_v659_ !== "function") {' +
    ' throw new Error("Remote function not found: continueBrandLifecycleDashboard_v659_");' +
    ' }\ncontinueBrandLifecycleDashboard_v659_();'
  );
}
