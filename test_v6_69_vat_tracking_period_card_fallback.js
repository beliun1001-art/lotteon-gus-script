...
function lotteKind(h){const e=end4(h.cardEnd4,h.cardNumber);if(e==='0126')return'TRIP';if(e==='0036')return'LIKIT';const s=compact(h.cardName);if(s.includes('tripto로카'))return'TRIP';if(s.includes('localikit'))return'LIKIT';return'';}
function filterLotte(rows,date){const expected=date<='2026-05-28'?'TRIP':'LIKIT';return rows.filter(h=>lotteKind(h)===expected);}
function filterSingle(rows,issuer){if(issuer==='KB국민카드')return rows.filter(h=>compact((h.cardName||'')+' '+(h.cardEnd4||'')).includes('heritage')||end4(h.cardEnd4,h.cardNumber)==='4091');if(issuer==='우리카드')return rows.filter(h=>compact((h.cardName||'')+' '+(h.cardEnd4||'')).includes('everypoint')||end4(h.cardEnd4,h.cardNumber)==='7680');return rows;}
let baseResult={status:'NO_MATCH',reason:'base no match'};
const sandbox={console,Object,compact_v660_:compact,text_v660_:text,digits_v660_:digits,normalizeCardCompany_v660_:normalizeCompany,classifyVatTrackingPayment_v666_:classify,trackingPaymentReasonPrefix_v666_:reason,normalizeVatCardEnd4_v667_:end4,isActiveMaster_v660_:m=>!/해지|정지/.test(text(m.status)),enrichHistoryFromMaster_v660_:(h,master)=>{const m=master.find(x=>normalizeCompany(x.company)===normalizeCompany(h.company)&&end4(x.cardEnd4,x.cardNumber)===end4(h.cardEnd4,h.cardNumber));return{company:h.company||(m&&m.company)||'',alias:(m&&m.alias)||'',cardName:(m&&m.cardName)||h.cardName||'',cardNumber:h.cardNumber||(m&&m.cardNumber)||'',cardEnd4:h.cardEnd4||(m&&m.cardEnd4)||''};},historyCandidateLabel_v660_:h=>[h.company,h.cardName,end4(h.cardEnd4,h.cardNumber),h.date].filter(Boolean).join(' / '),masterCandidateLabel_v660_:m=>[m.company,m.cardName,end4(m.cardEnd4,m.cardNumber)].filter(Boolean).join(' / '),isKakaoCardEvidence_v666_:h=>!!h.v666KakaoCard,filterLotteCardByOrderDate_v666_:(rows,date)=>filterLotte(rows,date),filterKnownSingleCard_v666_:(rows,issuer)=>filterSingle(rows,issuer),matchVatOrderCardCanonical_v664_:()=>Object.assign({},baseResult)};
