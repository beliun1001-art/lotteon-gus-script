 ...
 *
 * 이 파일은 Apps Script에 직접 붙여넣는 로컬 loader 최종본입니다.
 * GitHub의 Code.gs + Patch_v6_01_daily_filter_auto.gs bootstrap을 불러와 실행합니다.
 *
 * v1.4 핵심:
 * - 기존 LOTTEON 자동화 메뉴 유지
 * - LOTTEON 서식 메뉴 추가
 * - 열너비 자동조정 v6.23 wrapper 추가
 * - 부가세 신고자료 별도 생성 v6.22 wrapper 추가
 *
 * 사용 방법:
 * - Apps Script에 있는 기존 loader 코드를 이 파일 전체로 교체합니다.
 * - 저장 후 시트를 새로고침합니다.
 */
...
const LOTTEON_LOADER_VERSION = 'v1.4';
const LOTTEON_REMOTE_CACHE_KEY = 'LOTTEON_REMOTE_CODE_BUNDLE_V14';
const LOTTEON_REMOTE_CACHE_SECONDS = 60;
...
function testLotteonGitHubConnection() {
  const bundle = loadLotteonRemoteBundle_(true);
  ...
  SpreadsheetApp.getUi().alert(
    ...
    '버전 추정: ' + version + '\n' +
    '현재 구조: Code.gs + patch bootstrap 포함\n\n' +
    '상단에 LOTTEON 자동화 / LOTTEON 서식 메뉴가 보여야 정상입니다.'
  );
}

function clearLotteonGitHubCodeCache() {
  CacheService.getScriptCache().remove(LOTTEON_REMOTE_CACHE_KEY);
  PropertiesService.getScriptProperties().deleteProperty('LOTTEON_REMOTE_LAST_VERSION');
  SpreadsheetApp.getUi().alert('GitHub 코드 캐시를 초기화했습니다.\n\n이후 GitHub 코드 연결 테스트를 다시 실행하세요.');
}

function loadLotteonRemoteBundle_(forceFresh) {
  const cache = CacheService.getScriptCache();
  if (!forceFresh) {
    const cached = cache.get(LOTTEON_REMOTE_CACHE_KEY);
  ...
  cache.put(LOTTEON_REMOTE_CACHE_KEY, bundle, LOTTEON_REMOTE_CACHE_SECONDS);
...
function runRemoteFirstAvailable_(functionNames) {
  ...
  return eval(bundle + '\n\n; (function(){ var names = ' + namesLiteral + '; for (var i=0;i<names.length;i++){ if (typeof this[names[i]] === "function") return this[names[i]](); if (typeof eval(names[i]) === "function") return eval(names[i])(); } throw new Error("Remote function not found: " + names.join(", ")); }).call(this);');
}
...
function runPendingChangesApproval() {
  return runRemoteFunctionByName_('runPendingChangesApproval');
}

function refreshDashboardFastOnly() {
  return runRemoteFunctionByName_('refreshDashboardFastOnly');
}

function runDailyFilterCountsOnceManual() {
  return runRemoteFirstAvailable_(['runDailyFilterCountsOnceManual', 'runDailyFilterCountsStep_', 'refreshFilterCountsFast']);
}

function showDailyFilterCountsStatus() {
  return runRemoteFirstAvailable_(['showDailyFilterCountsStatus', 'showDailyFilterCountsStatus_v601', 'showFilterCountsStatus']);
}

function createRetransmitLogSheet() {
  return runRemoteFunctionByName_('createRetransmitLogSheet');
}

function refreshCoreSummaryAndDashboardWithRetransmitLogDates() {
  return runRemoteFunctionByName_('refreshCoreSummaryAndDashboardWithRetransmitLogDates');
}

function showOperationSheetsOnly() {
  return runRemoteFirstAvailable_(['showOperationSheetsOnly', 'showOnlyOperationSheets', 'hideNonOperationSheets']);
}

function showAllSheets() {
  return runRemoteFirstAvailable_(['showAllSheets', 'restoreAllSheetsVisible']);
}

function saveApiCredentials() {
  return runRemoteFirstAvailable_(['saveApiCredentials', 'saveApiCredentialsMenu']);
}

function testApiConnection() {
  return runRemoteFirstAvailable_(['testApiConnection', 'testApiConnectionMenu']);
}

function startChangeDetection() {
  return runRemoteFirstAvailable_(['startChangeDetection', 'startChangeDetectionTrigger']);
}

function stopChangeDetection() {
  return runRemoteFirstAvailable_(['stopChangeDetection', 'stopChangeDetectionTrigger']);
}

function resetChangeDetectionFlags() {
  return runRemoteFirstAvailable_(['resetChangeDetectionFlags', 'resetChangeDetectionFlag']);
}

function resetFilterListResumeState() {
  return runRemoteFirstAvailable_(['resetFilterListResumeState', 'resetFilterListResume']);
}

function generateVatReportsFullSeparated_v622() {
  return runRemoteFunctionByName_('generateVatReportsFullSeparated_v622');
}

function runColumnWidthAutoAdjustStep_v623() {
  return runRemoteFunctionByName_('runColumnWidthAutoAdjustStep_v623');
}

function showColumnWidthAutoAdjustStatus_v623() {
  return runRemoteFunctionByName_('showColumnWidthAutoAdjustStatus_v623');
}

function resetColumnWidthAutoAdjust_v623() {
  return runRemoteFunctionByName_('resetColumnWidthAutoAdjust_v623');
}

function applyDisplayStandardsOnlyFast_v623() {
  return runRemoteFunctionByName_('applyDisplayStandardsOnlyFast_v623');
}
