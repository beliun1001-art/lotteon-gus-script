/**
 * PR #25 v6.66 trigger-based continuation runner R7.
 *
 * - VAT financial mapping remains A:AC.
 * - Tracking/payment headers are searched across every source column.
 * - Uses persisted state and one-minute automatic continuation triggers.
 * - No modal alert calls.
 *
 * Requires SmokeRunner_v1_15_pr25_tracking_payment.gs (R5 helpers)
 * in the same Apps Script project.
 */
const PR25_R7_VERSION = 'v1.18-PR25-CONTINUATION-R7';
const PR25_R7_STATE_KEY = 'PR25_TRACKING_CONTINUATION_R7_STATE';
const PR25_R7_HANDLER = 'continuePr25TrackingContinuationR7';
const PR25_R7_STATUS_SHEET = 'PR25_실행상태';
const PR25_R7_DELAY_MS = 60 * 1000;
const PR25_R7_CORE_COLS = 29;

function startPr25TrackingContinuationR7() {
  const lock = LockService.getScriptLock();
  if (!lock.tryLock(5000)) return { ok: false, busy: true };
  try {
    pr25r7_assertHelpers_();
    const ss = SpreadsheetApp.getActive();
    pr25r7_assertSheets_(ss);
    pr25r7_clearTriggers_();

    const source = ss.getSheetByName('매출데이터_붙여넣기');
    const found = pr25r7_discoverPaymentColumns_(source);
    ...
    pr25r7_saveState_(state);
    pr25r7_schedule_(state);
    pr25r7_writeStatus_(ss, state, '원본 전체 열 검색 완료; 트래킹 결제수단 백필 예약');
    ss.toast('PR25 R7 자동 이어실행을 시작했습니다.', 'PR25 R7', 8);
...
function continuePr25TrackingContinuationR7() {
  const lock = LockService.getScriptLock();
  if (!lock.tryLock(5000)) return { ok: false, busy: true };
  try {
    const state = pr25r7_getState_();
    ...
    try {
      if (state.phase === 'backfill') {
        const result = pr25r7_backfillTrackingPayment_(ss, state);
        ...
        pr25r7_saveState_(state);
        pr25r7_schedule_(state);
        pr25r7_writeStatus_(ss, state, '트래킹 결제수단 백필 완료; 카드 재매칭 예약');
      ...
      if (state.phase === 'match') {
        ...
        pr25r7_saveState_(state);
        pr25r7_clearTriggers_();
        pr25r7_writeStatus_(ss, state, 'PR25 트래킹 결제수단 카드 재매칭 완료');
        ss.toast('PR25 R7 카드 재매칭이 완료되었습니다.', 'PR25 R7', 8);
      ...
      pr25r7_saveState_(state);
      pr25r7_clearTriggers_();
      pr25r7_writeStatus_(ss, state, '오류로 중단');
      try { ss.toast('PR25 R7 작업이 중단되었습니다. PR25_실행상태를 확인하세요.', 'PR25 R7', 10); } catch (ignore) {}
      throw e;
    }
  } finally {
    lock.releaseLock();
...
function pr25r7_backfillTrackingPayment_(ss, state) {
  const source = ss.getSheetByName('매출데이터_붙여넣기');
  const detail = ss.getSheetByName('부가세_신고자료');
  ...
  if (source.getLastColumn() < PR25_R7_CORE_COLS) {
  ...
  const sourceValues = source.getRange(1, 1, lastRow, PR25_R7_CORE_COLS).getValues();
  ...
  let leftovers = 0;
  Object.keys(queues).forEach(function(key) { leftovers += queues[key].length; });
  if (unmatched.length || leftovers) {
    throw new Error('안전검증 실패: 상세 미매칭 ' + unmatched.length + '건 / 원본 잔여 ' + leftovers + '건. 아무 값도 쓰지 않았습니다.');
  }
  if (paymentRows < 1 || generatedNonblank < 1) {
    throw new Error('발견한 결제수단 헤더의 값이 0건입니다. 아무 값도 쓰지 않았습니다.');
  }
...
function pr25r7_discoverPaymentColumns_(source) {
  const lastColumn = source.getLastColumn();
  const headers = source.getRange(1, 1, 1, lastColumn).getDisplayValues()[0] || [];
  let trackingIndex = pr25r7_findHeader_(headers, ['트래킹 번호','트래킹번호','tracking number','trackingnumber']);
  let fallbackIndex = pr25r7_findHeader_(headers, ['결제수단','결제정보','결제방법','카드사','결제수단/카드사','결제수단(카드사)','구매결제수단']);

  if (trackingIndex < 0) {
    for (let i = 0; i < headers.length; i++) {
      const compact = pr25r7_compactHeader_(headers[i]);
  ...
  headers.forEach(function(header, index) {
    const compact = pr25r7_compactHeader_(header);
...
function pr25r7_findHeader_(headers, names) {
  for (let n = 0; n < names.length; n++) {
    const target = pr25r7_compactHeader_(names[n]);
    for (let i = 0; i < headers.length; i++) {
      if (pr25r7_compactHeader_(headers[i]) === target) return i;
    }
  }
  return -1;
}

function pr25r7_compactHeader_(value) {
  return String(value == null ? '' : value).toLowerCase().replace(/[\s._()\[\]{}\-\/]/g, '');
}

function resetPr25TrackingContinuationR7() {
  pr25r7_clearTriggers_();
  PropertiesService.getScriptProperties().deleteProperty(PR25_R7_STATE_KEY);
  const ss = SpreadsheetApp.getActive();
  pr25r7_writeStatus_(ss, {
    version: PR25_R7_VERSION,
    status: 'reset',
    phase: '-',
    nextRunScheduled: false,
...
function pr25r7_assertHelpers_() {
  const required = [
...
function pr25r7_assertSheets_(ss) {
...
function pr25r7_getState_() {
  const raw = PropertiesService.getScriptProperties().getProperty(PR25_R7_STATE_KEY);
  if (!raw) return null;
  try { return JSON.parse(raw); } catch (e) { return null; }
}

function pr25r7_saveState_(state) {
  PropertiesService.getScriptProperties().setProperty(PR25_R7_STATE_KEY, JSON.stringify(state));
}

function pr25r7_schedule_(state) {
  pr25r7_clearTriggers_();
  ScriptApp.newTrigger(PR25_R7_HANDLER).timeBased().after(PR25_R7_DELAY_MS).create();
  state.nextRunScheduled = true;
  state.updatedAt = new Date().toISOString();
  pr25r7_saveState_(state);
}

function pr25r7_clearTriggers_() {
  ScriptApp.getProjectTriggers().forEach(function(trigger) {
    try {
      const handler = trigger.getHandlerFunction();
      if (handler === PR25_R7_HANDLER || handler === 'continuePr25TrackingContinuationR6') ScriptApp.deleteTrigger(trigger);
    } catch (e) {}
  });
}

function pr25r7_writeStatus_(ss, state, message) {
  const sheet = ss.getSheetByName(PR25_R7_STATUS_SHEET) || ss.insertSheet(PR25_R7_STATUS_SHEET);
  ...
  const rows = [
    ['항목','값'],
    ['버전',state.version || PR25_R7_VERSION],
    ['상태',state.status || ''],
    ['단계',state.phase || ''],
    ['메시지',message || ''],
