/**
 * Issue #54 v1.0 corrected VAT 1,355-order read-only card rematch preview.
 *
...
var LOTTEON_REMOTE_TASK = {
  id: 'ISSUE54-v1.0-20260813',
  title: 'corrected VAT 1,355주문 카드매칭 전체 read-only 재검증',
  enabled: true,
...
function issue54LoadTransformedBase_() {
  ...
  src = issue54ReplaceAll_(src, 'v1.0-ISSUE54-CARD-REMATCH-PREVIEW', 'v1.0-ISSUE54-CORRECTED-CARD-REMATCH-PREVIEW');
  src = issue54ReplaceAll_(src, '현재 VAT 1,893주문 카드매칭 전체 preview', 'corrected VAT 1,355주문 카드매칭 전체 preview');
  src = issue54ReplaceAll_(src, '현재 VAT 1,893주문 전체 카드매칭 preview 사전검증 시작', 'corrected VAT 1,355주문 카드매칭 preview 사전검증 시작');
  src = issue54ReplaceAll_(src, '현재 VAT 1,893주문 카드매칭 전체 preview 재계산 완료', 'corrected VAT 1,355주문 카드매칭 전체 preview 재계산 완료');
  ...
  src = issue54ReplaceAll_(src, "id: 'ISSUE54-v1.0-20260812'", "id: 'ISSUE54-v1.0-20260813'");
  src = issue54ReplaceAll_(src, "outputSheet: 'ISSUE54_카드매칭전체PREVIEW'", "outputSheet: 'ISSUE54_카드매칭전체PREVIEW'");
  src = issue54ReplaceAll_(src, "statusSheet: 'ISSUE54_실행상태'", "statusSheet: 'ISSUE54_실행상태'");
