var LOTTEON_REMOTE_TASK={id:'ISSUE68-v1.0-20260814',title:'corrected VAT 부가세_기간별 guarded production rebuild',enabled:true,statusSheet:'ISSUE68_반영상태'};
var I68_VERSION='v1.0-ISSUE68-CORRECTED-VAT-PERIOD-PRODUCTION';
...
function runLotteonRemoteTaskStartRemote_(){
  ...
  var wrote=false,backupReady=false,periodFilterMeta=null,before=null,prep=null,oldPeriod=null;
  try{
    ...
    if(bizPreview.getLastRow()!==20||bizPreview.getLastColumn()!==24)throw new Error('Issue67 사업자 preview 구조 불일치 '+bizPreview.getLastRow()+'x'+bizPreview.getLastColumn());
    ...
    prep=ss.insertSheet(I68_PREP);i68EnsureGrid_(prep,matrix.values.length,matrix.values[0].length);
    ...
    var bizSrc=bizPreview.getRange(1,1,20,24),bizDst=prep.getRange(2,1,20,24);
    ...
    var oldFrozenRows=period.getFrozenRows(),oldFrozenCols=period.getFrozenColumns(),oldWidths=[];for(var cw=1;cw<=period.getMaxColumns();cw++)oldWidths.push(period.getColumnWidth(cw));
    ...
    var prodPeriod=i68ReadPeriodBlock_(period,matrix.detailStart);i68Assert_(prodPeriod.halfTotals,periodAgg.halfTotals);i68Assert_(prodPeriod.monthTotals,periodAgg.monthTotals);if(prodPeriod.periodUnknown!==0)throw new Error('운영 기간미확인 '+prodPeriod.periodUnknown);
    i68CheckProtected_(ss,before);
    ...
    if(wrote&&backupReady){
      try{
        ...
        if(br&&bc){backupSh.getRange(1,1,br,bc).copyTo(prod.getRange(1,1,br,bc),SpreadsheetApp.CopyPasteType.PASTE_FORMAT,false);prod.getRange(1,1,br,bc).setNumberFormats(backupSh.getRange(1,1,br,bc).getNumberFormats());prod.getRange(1,1,br,bc).setValues(backupSh.getRange(1,1,br,bc).getValues());}
...
function i68BuildMatrix_(bizValues,periodRows){
  var bizHeaders=bizValues[0],summary=bizValues.slice(1),detailHeaders=['집계구분','신고연도','반기','신고월','쿠팡계정ID','사업자등록번호','주문건수','순수매출액','정산기준금액','마켓수수료','매입금액','매출부가세','매입부가세','납부예상부가세','예상이익','부가세반영예상이익'];
  if(bizHeaders.length!==24||summary.length!==19)throw new Error('Issue67 preview 헤더/행수 불일치');
  ...
  values.push(bizHeaders.concat(new Array(maxCols-bizHeaders.length).fill('')));
  summary.forEach(function(r){values.push(r.concat(new Array(maxCols-r.length).fill('')));});
  while(values.length<detailStart-1)values.push(blankRow());
  values.push(detailHeaders.concat(new Array(maxCols-detailHeaders.length).fill('')));
  periodRows.forEach(function(r){values.push(r.concat(new Array(maxCols-r.length).fill('')));});
...
function i68RestoreFilterFromSheet_(dst,src){var m=i68CaptureFilter_(src);i68RestoreFilter_(dst,m);}
...
function i68Need_(ss,n){var sh=ss.getSheetByName(n);if(!sh)throw new Error('필수 시트 없음 '+n);return sh;}function i68Ensure_(ss,n){return ss.getSheetByName(n)||ss.insertSheet(n);}function i68EnsureGrid_(sh,r,c){if(sh.getMaxRows()<r)sh.insertRowsAfter(sh.getMaxRows(),r-sh.getMaxRows());if(sh.getMaxColumns()<c)sh.insertColumnsAfter(sh.getMaxColumns(),c-sh.getMaxColumns());}
function i68Find_(h,names){for(var n=0;n<names.length;n++){var w=i68Compact_(names[n]);for(var i=0;i<h.length;i++)if(i68Compact_(h[i])===w)return i;}return-1;}function i68Text_(v){return String(v==null?'':v).trim();}function i68Compact_(v){return i68Text_(v).replace(/\s/g,'');}function i68Num_(v){var n=Number(typeof v==='number'?v:i68Text_(v).replace(/[,원\s]/g,''));return isNaN(n)?0:n;}function i68Key_(a,o){a=i68Text_(a).toLowerCase();o=i68Text_(o).toLowerCase().replace(/[^0-9a-z가-힣]/g,'');return a&&o?a+'|'+o:'';}
