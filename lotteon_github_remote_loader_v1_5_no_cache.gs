 ...
 * - LOTTEON 서식 메뉴 유지
 * - 열너비 자동조정 v6.23 wrapper 유지
 * - 부가세 신고자료 별도 생성 v6.22 wrapper 유지
 * - CacheService에 대용량 원격 bundle을 저장하지 않음
 *   → Exception: 인수(value)가 너무 큽니다 방지
 *
 * 사용 방법:
...
const LOTTEON_LOADER_VERSION = 'v1.5';
...
function clearLotteonGitHubCodeCache() {
  // v1.5부터는 대용량 bundle을 CacheService에 저장하지 않습니다.
  // 과거 v1.4 이하에서 남았을 수 있는 캐시만 제거합니다.
  try { CacheService.getScriptCache().remove('LOTTEON_REMOTE_CODE_BUNDLE_V14'); } catch (e) {}
  try { CacheService.getScriptCache().remove('LOTTEON_REMOTE_CODE_BUNDLE_V13'); } catch (e) {}
  try { CacheService.getScriptCache().remove('LOTTEON_REMOTE_CODE_BUNDLE'); } catch (e) {}
  PropertiesService.getScriptProperties().deleteProperty('LOTTEON_REMOTE_LAST_VERSION');
  SpreadsheetApp.getUi().alert('GitHub 코드 캐시를 초기화했습니다.\n\nv1.5부터는 대용량 코드를 캐시에 저장하지 않습니다.');
}
