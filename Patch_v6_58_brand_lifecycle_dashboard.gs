...
function aggregateLifecycleFilters_v658_(base) {
  ...
  (base && base.rows || []).forEach(function(row) {
    ...
    var b = result.byBrand[item.key] || (result.byBrand[item.key] = { brand:item.brand, key:item.key, collectCount:0, filters:[], accountIds:{}, earliestCreateDate:'', earliestRecentDate:'', representativeFilterName:'', representativeAccountId:'', diagnosticMemo:'' });
...
function decideLifecycle_v658_(elapsed, products30, sales30, firstDate) { if (!firstDate) return { status:'유지', reason:'최초수집일 미확인' }; if (elapsed == null) return { status:'유지', reason:'미래 최초수집일' }; if (elapsed < 30) return { status:'유지', reason:'관찰기간 미도달' }; if (elapsed >= 45 && products30 <= 0 && sales30 <= 0) return { status:'퇴출', reason:'45일 이상 매출/품목 없음' }; if (products30 >= 10 || sales30 >= 1000000) return { status:'확장', reason:products30 >= 10 ? '30일환산 품목수 기준 충족' : '30일환산 매출 기준 충족' }; return { status:'유지', reason:'운영 기준 유지' }; }

function buildBrandLifecycleRows_v658_(salesAgg, filterAgg, today) {
  var salesByKey = {};
  ...
  return Object.keys(keys).map(function(key) {
    ...
    var products = Number(s.salesProductCount || 0), sales = Number(s.netSalesAmount || s.salesAmount || 0), divisor = Math.max(elapsed == null ? 1 : elapsed, 1);
    var products30 = products / divisor * 30, sales30 = sales / divisor * 30, decision = decideLifecycle_v658_(elapsed, products30, sales30, first);
...
function appendBrandLifecycleDashboard_v658_(salesAgg, filterAgg) {
  ...
  var rows = buildBrandLifecycleRows_v658_(salesAgg || {}, filterAgg || {}, lifecycleToday_v658_());
