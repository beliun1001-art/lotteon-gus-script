...
function lifecycleDefaultRules_v658_() { return { expansionMinDays:30, exitMinDays:45, expansionProducts30:10, expansionSales30:1000000, exitProducts30:0, exitSales30:0 }; }
...
function loadLifecycleRules_v658_(ss) {
  ...
  var map = { 확장최소경과일:'expansionMinDays', 퇴출최소경과일:'exitMinDays', 확장30일환산매출품목수:'expansionProducts30', 확장30일환산순수매출액:'expansionSales30', 퇴출최대30일환산매출품목수:'exitProducts30', 퇴출최대30일환산순수매출액:'exitSales30' };
...
function applyLifecycleConditionalFormats_v658_(sheet, row, count) { if (!count || typeof SpreadsheetApp === 'undefined') return; var range = sheet.getRange(row, 13, count, 1), rules = sheet.getConditionalFormatRules(); lifecycleStatusFormatSpecs_v658_().forEach(function(spec) { rules.push(SpreadsheetApp.newConditionalFormatRule().whenTextEqualTo(spec.text).setBackground(spec.color).setRanges([range]).build()); }); sheet.setConditionalFormatRules(rules); }
