/**
 * LOTTEON Google Sheets Apps Script GitHub Remote Loader v1.7
 *
 * v1.7 핵심:
 * - v1.6 no-cache 구조 유지
 * - LOTTEON 자동화 / LOTTEON 서식 메뉴 유지
 * - GitHub patch bootstrap은 Patch_v6_24_bootstrap_auto_continue.gs 사용
 * - ScriptApp.getProjectTriggers / newTrigger / deleteTrigger 권한을 로컬 loader에 명시
 *   → 원격 eval 코드에서 필터별_상품수 자동 갱신 또는 열너비 자동 이어실행이 트리거를 사용할 때
 *      "지정된 권한으로는 ScriptApp.getProjectTriggers을 호출할 수 없습니다" 오류 방지
 *
 * 사용 방법:
...
const LOTTEON_LOADER_VERSION = 'v1.7';
...
function authorizeLotteonScriptAppScope() {
  // Apps Script가 script.scriptapp 권한을 정적으로 인식하도록 로컬 loader에 직접 둡니다.
  const triggers = ScriptApp.getProjectTriggers();

  // newTrigger/deleteTrigger 권한도 같은 scope지만, 실제 호출 경로를 명시해 권한 승인을 확실히 유도합니다.
  ...
  try {
    ...
    // 동일 함수 트리거 생성이 제한될 수 있으므로 getProjectTriggers 권한만 확인해도 통과로 봅니다.
  }
  if (dummy) {
...
function lotteonDummyTriggerScopeHandler_() {
  // 권한 요청용 더미 함수입니다. 실제 작업은 하지 않습니다.
}
...
function testLotteonGitHubConnection() {
  ...
  SpreadsheetApp.getUi().alert(
    ...
    '필터별_상품수 자동 갱신과 열너비 자동 이어실행 트리거 사용 가능 상태입니다.'
  );
}
