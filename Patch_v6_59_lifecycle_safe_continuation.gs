...
function scheduleLifecycleTriggerOnly_v659_(delayMs) { clearLifecycleTriggers_v659_(); ScriptApp.newTrigger(LOTTEON_V659_HANDLER).timeBased().after(delayMs || 1000).create(); }
function scheduleLifecycleTrigger_v659_(state, delayMs) { scheduleLifecycleTriggerOnly_v659_(delayMs); state.triggerScheduled = true; saveLifecycleState_v659_(state); return state; }
...
function continueBrandLifecycleDashboard_v659_() {
  var lock = LockService.getScriptLock(); if (!lock.tryLock(5000)) { var busyState = getLifecycleState_v659_(); if (busyState && busyState.status !== 'done' && busyState.status !== 'failed') scheduleLifecycleTriggerOnly_v659_(LOTTEON_V659_SNAPSHOT_WATCHDOG_DELAY_MS); return { ok:false, busy:true, rescheduled:!!busyState }; }
